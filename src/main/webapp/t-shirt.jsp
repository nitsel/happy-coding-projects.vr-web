...
<div class="entry">
	<div class="float-info">
        ...
        <ul class="ul-properties">
        	<li><span>Name: </span><a style="text-transform:capitalize" name="#">${tee.name}</a></li>
			<li><span>Price:</span>${tee.price } </li>
        	<li><span>Faboric Details:</span>
        		<ul><li>${tee.features} </li></ul>
                </li>
            <li><span>Description:</strong> ${tee.description}</li>
        </ul>
	</div>
	<div class="float-rating">
		...
		<form id="ratingForm" action="#" method="get" onSubmit="return submit_rating()">
			...
			<input name="star1" type="radio" value="5" class="star" title="Very Good"/><br/>
			<p>Comments&nbsp;&nbsp;(optional):<textarea id="comments" name="comments" cols="43" rows="5"></textarea></p>
...
<div class="entry">
	...
	<div>	 
		...
		<div class="stars">
            ...
            <span class="block-rating">${tee.avgRating}</span>(Based on ${tee.numRating} Reviews)
	  </div>
      <p><span class="block-reco">${tee.recommendation}</span>of respondents would recommend this to a friend.
		<table>
			<tr>
			  <th>Pros</th>
			  <th>Cons</th>
			  <th>Best Uses</th></tr>
			<tr><td>${tee.pros }</td><td>${tee.cons }</td><td>${tee.bestUses }</td></tr>
		</table>
        <ul class="ul-properties">
...
<c:forEach var="r" items="${requestScope.reviews}">
	<div class="entry">
		<div>
			...
			<input name="star4" type="radio" class="star" value="5" disabled="disabled" <c:if test="${r.rating==5}">checked="checked"</c:if>/> </form>
            <span class="block-rating">${r.rating}</span>
            <strong>${r.title}</strong><span class="vDate">${r.vDate }</span> <br/>
		</div>
        <ul class="ul-properties">
        	<li>By <strong>${r.userName}</strong> from <strong>${r.userLocation}</strong> </li>
        	...
        	<li>
            	<table>
                    <tr>
                      ...
                      <th>Cons</th>
                      <th>Best Uses</th></tr>
                    <tr><td>${r.pros }</td><td>${r.cons }</td><td>${r.bestUses }</td></tr>
                </table>
            </li>
            <c:if test="${!empty r.comments }">
            	<li><span>Comments about <em>${tee.name}</em></span><br/> ${r.comments }</li>
            </c:if>
            <c:if test="${!empty r.services }">
            	<li><span>Service and delivery comments</span><br/> ${r.services }</li>
            </c:if>
